...
bool Database::Connect() {
    ...
    const QString DB_NAME = QDir::cleanPath(QDir::homePath() + QDir::separator() + "hmis.sqlite3");
...
bool Database::saveNewRow(const NewHMISData& data) {
    ...
    checkQuery.prepare("SELECT COUNT(*) FROM hmis WHERE ip_number = :ip_number AND month = :month AND year = :year;");
    ...
    if (!checkQuery.exec()) {
        QMessageBox::critical(nullptr,
                              "DB ERROR",
                              "Database error while checking for duplicate IP number:" + checkQuery.lastError().text());
        return false;
    }

    if (checkQuery.next() && checkQuery.value(0).toInt() > 0) {
        QMessageBox::critical(nullptr, "Duplicate IP Number", "IP number already exists for the given month and year.");
        return false;
    }
...
bool Database::readFromBackup(QWidget* parent) {
    QString fileName = QFileDialog::getOpenFileName(parent, "Open database backup");
