...
void MainWindow::connectSignals() {
    ...
    connect(ui->actionBackup, &QAction::triggered, this, &MainWindow::onDatabaseBackup);
    connect(ui->actionRestore, &QAction::triggered, this, &MainWindow::onDatabaseRestore);
}
...
void MainWindow::populateAttendances(int year, int month) {
    ...
    for (row = 0; row < 2; row++) {
        ...
        ui->tableAttendances->setItem(
            row, 0, new QTableWidgetItem(QString::number(map[newAttendance][ZERO_TO_TWENTY_EIGHT_DAYS][MALE])));

        ui->tableAttendances->setItem(
            row, 1, new QTableWidgetItem(QString::number(map[newAttendance][ZERO_TO_TWENTY_EIGHT_DAYS][FEMALE])));

        // Infants and toddlers
        ui->tableAttendances->setItem(
            row, 2, new QTableWidgetItem(QString::number(map[newAttendance][TWENTY_NINE_DAYS_TO_FOUR_YEARS][MALE])));

        ui->tableAttendances->setItem(
            row, 3, new QTableWidgetItem(QString::number(map[newAttendance][TWENTY_NINE_DAYS_TO_FOUR_YEARS][FEMALE])));

        // school-going children
        ui->tableAttendances->setItem(
            row, 4, new QTableWidgetItem(QString::number(map[newAttendance][FIVE_TO_NINE_YEARS][MALE])));
        ui->tableAttendances->setItem(
            row, 5, new QTableWidgetItem(QString::number(map[newAttendance][FIVE_TO_NINE_YEARS][FEMALE])));

        // Adolescents
        ui->tableAttendances->setItem(
            row, 6, new QTableWidgetItem(QString::number(map[newAttendance][TEN_TO_NINETEEN_YEARS][MALE])));
        ui->tableAttendances->setItem(
            row, 7, new QTableWidgetItem(QString::number(map[newAttendance][TEN_TO_NINETEEN_YEARS][FEMALE])));

        // Adults above 20
        ui->tableAttendances->setItem(
            row, 8, new QTableWidgetItem(QString::number(map[newAttendance][TWENTY_YEARS_AND_ABOVE][MALE])));
        ui->tableAttendances->setItem(
            row, 9, new QTableWidgetItem(QString::number(map[newAttendance][TWENTY_YEARS_AND_ABOVE][FEMALE])));
    }
}
...
void MainWindow::populateDiagnoses(int year, int month) {
    ...
    for (row = 0; row < diagnoses.size(); row++) {
        ...
        ui->tableDiagnoses->setItem(
            row, column, new QTableWidgetItem(QString::number(map[dx][ZERO_TO_TWENTY_EIGHT_DAYS][MALE])));
        ui->tableDiagnoses->setItem(
            row, column + 1, new QTableWidgetItem(QString::number(map[dx][ZERO_TO_TWENTY_EIGHT_DAYS][FEMALE])));

        // Infants and toddlers
        ui->tableDiagnoses->setItem(
            row, column + 2, new QTableWidgetItem(QString::number(map[dx][TWENTY_NINE_DAYS_TO_FOUR_YEARS][MALE])));
        ui->tableDiagnoses->setItem(
            row, column + 3, new QTableWidgetItem(QString::number(map[dx][TWENTY_NINE_DAYS_TO_FOUR_YEARS][FEMALE])));

        // school-going children
        ui->tableDiagnoses->setItem(
            row, column + 4, new QTableWidgetItem(QString::number(map[dx][FIVE_TO_NINE_YEARS][MALE])));
        ui->tableDiagnoses->setItem(
            row, column + 5, new QTableWidgetItem(QString::number(map[dx][FIVE_TO_NINE_YEARS][FEMALE])));

        // Adolescents
        ui->tableDiagnoses->setItem(
            row, column + 6, new QTableWidgetItem(QString::number(map[dx][TEN_TO_NINETEEN_YEARS][MALE])));
        ui->tableDiagnoses->setItem(
            row, column + 7, new QTableWidgetItem(QString::number(map[dx][TEN_TO_NINETEEN_YEARS][FEMALE])));

        // Adults above 20
        ui->tableDiagnoses->setItem(
            row, column + 8, new QTableWidgetItem(QString::number(map[dx][TWENTY_YEARS_AND_ABOVE][MALE])));
        ui->tableDiagnoses->setItem(
            row, column + 9, new QTableWidgetItem(QString::number(map[dx][TWENTY_YEARS_AND_ABOVE][FEMALE])));
    }
}
...
void MainWindow::onDatabaseBackup() {
    const QString dirName = QFileDialog::getExistingDirectory(this, "Backup directory", QString());
    if (dirName.isEmpty()) {
        return;
    }
    ...
    if (QFile::exists(databasePath)) {
        ...
        QString backupName =
            QDir::cleanPath(dirName + QDir::separator() + fileInfo.baseName() + "." + fileInfo.suffix());

        QFile srcFile(databasePath);

        if (!srcFile.copy(backupName)) {
            srcFile.close();
            QMessageBox::warning(this, "IO Error", "Error copying backup to disk: " + srcFile.errorString());
            return;
        }
...
void MainWindow::onViewRegister() {
    ...
    auto* registerDialog = new Register(this);
